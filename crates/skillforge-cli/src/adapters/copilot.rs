...
fn config_path() -> Option<PathBuf> {
    std::env::var_os("HOME").map(|h| PathBuf::from(h).join(".copilot/mcp-config.json"))
}
...
impl Agent for Copilot {
    ...
    fn detect(&self) -> bool {
        if let Some(home) = std::env::var_os("HOME") {
            PathBuf::from(home).join(".copilot").exists()
        } else {
            false
        }
    }
    ...
    fn uninstall(&self, skill: &SkillRef) -> Result<Status> {
        ...
        remove_mcp_server_copilot(&path, &server_key(skill))
    }
    ...
    fn is_linked(&self, skill: &SkillRef) -> Result<bool> {
        ...
        has_mcp_server_copilot(&path, &server_key(skill))
    }
    ...
    fn uninstall_mux(&self) -> Result<Status> {
        ...
        remove_mcp_server_copilot(&path, MUX_SERVER_NAME)
    }
}

/// Upsert an MCP server into Copilot's config with the format it expects
/// (including "type": "local", "env": {}, and "tools": ["*"])
fn upsert_mcp_server_copilot(
    path: &Path,
    name: &str,
    binary: &Path,
    args: &[String],
...
fn remove_mcp_server_copilot(path: &Path, name: &str) -> Result<Status> {
    if !path.exists() {
...
fn has_mcp_server_copilot(path: &Path, name: &str) -> Result<bool> {
    if !path.exists() {
        return Ok(false);
    }
